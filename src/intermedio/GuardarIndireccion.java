...
public class GuardarIndireccion extends InstruccionTresDirecciones {
    ...
    public String toMachineCode() {
        DeclaracionArray declArray = (DeclaracionArray) this.getSegundo().getValor();
    	BloqueInstrucciones bI = new BloqueInstrucciones();
        bI.add(Instruccion.nuevaInstruccion(super.toMachineCode()));

        bI.add(this.getSegundo().loadStringDescriptorVariable(AddressRegister.A5));
        bI.add(new Instruccion(OpCode.MOVE, Size.L, Indireccion.__(4, AddressRegister.A5), AddressRegister.A5));
        bI.add(this.getTercero().load(DataRegister.D1));
        ...
        bI.add(this.getPrimero().load(DataRegister.D2));
